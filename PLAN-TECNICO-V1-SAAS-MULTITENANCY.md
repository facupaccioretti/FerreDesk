...
- cuenta global minima en `public` vinculada a un unico usuario tenant operativo;
- setup inicial obligatorio;
- trial persistido y bloqueante;
- Render como proveedor de deploy;
- Resend como proveedor de email transaccional;
- Cloudflare R2 para pgdump, logos de negocio y certificados/claves ARCA;
- archivos de tenant correctamente aislados y objetos sensibles privados por defecto;
- backup por tenant almacenado en R2;
- staging/preview con wildcard domain;
- compatibilidad máxima posible con el código actual.

### 2.1 Decisiones vigentes 2026-06-16

Este plan reemplaza cualquier referencia historica a Railway por Render.

Decisiones operativas vigentes:

- Render es el proveedor de despliegue y base PostgreSQL gestionada para beta.
- Resend es el proveedor de email transaccional para activacion y password reset.
- Cloudflare R2 es el storage aprobado para pgdump, logos de negocio y certificados/claves ARCA.
- La identidad global de V1 es transitoria: una cuenta global en `public` queda conectada a un unico usuario tenant. El usuario tenant sigue siendo el usuario operativo del ERP.
- No se implementa en V1 un modelo completo de usuarios globales multi-tenant ni multi-empresa.
- Las migraciones productivas deben ejecutarse como paso de release/predeploy o job controlado, no como responsabilidad permanente del proceso web.
...
- creación automática de cuenta global minima y vinculo a un unico usuario tenant;
- creación automática de `Ferreteria` y `Sucursal`;
- setup mínimo obligatorio;
- aislamiento de media de tenant;
- backup por schema almacenado en R2;
- email transaccional real con Resend;
...
9. Entorno local, Render y staging
10. Cloud readiness: Render, Resend y R2
...
- cuenta global minima;
- referencia explicita al tenant asignado;
- referencia textual al usuario tenant vinculado, sin FK cross-schema;
- token puente efimero de un solo uso si el login publico salta al subdominio tenant.
...
### 9.4.1 Cuenta global transitoria

Debe crearse una cuenta global en `public` vinculada a ese admin tenant.

Reglas:

- una cuenta global solo puede estar vinculada a un tenant en V1;
- una cuenta global solo puede resolver un usuario tenant;
- no debe reemplazar `AUTH_USER_MODEL`;
- no debe forzar refactor de permisos del ERP;
- debe permitir evolucionar despues a usuarios globales reales sin perder datos de beta.
...
- el login publico puede resolver la cuenta global y saltar al tenant correcto;
...
- certificados, claves ARCA y pgdump no quedan publicos;
...
- el archivo resultante debe subirse a Cloudflare R2;
- la clave R2 debe incluir schema o identificador estable del tenant;
- los pgdump deben ser privados por defecto y no exponerse por URL publica.
...
- mensajes al usuario;
- pruebas en Render/staging;
- subida, lectura autorizada y denegacion de acceso cruzado en R2.

### 12.6 Criterio de aceptación de fase

- backup desde tenant A no contiene datos de tenant B.
- backup desde tenant A queda en R2 bajo path tenant-safe.
- tenant B no puede obtener ni solicitar el backup de tenant A.
...
El register historico no debe crear usuarios sueltos.  
Debe reemplazarse por onboarding SaaS con cuenta global transitoria + usuario tenant unico:
...
- creacion de cuenta global en `public`;
- creacion de usuario tenant operativo;
- vinculo estricto 1 cuenta global -> 1 usuario tenant.
...
- una cuenta global puede registrarse como negocio;
- se crea tenant;
- se crea usuario tenant vinculado;
...
## 14. Fase 9 — Entorno local, Render y staging
...
- deploy definido por un unico camino Render, preferentemente Docker;
- contexto Docker verificado igual al que Render usara;
- frontend React integrado en el artefacto servido por Django;
- `render.yaml` sin ruta productiva alternativa contradictoria.

### 14.4 Migraciones productivas

Las migraciones de `public` y tenants deben ejecutarse en un paso controlado de release/predeploy o job operativo.

No se acepta que el proceso web dependa permanentemente de correr migraciones en cada arranque, porque en produccion eso puede provocar:

- locks durante deploy;
- ejecuciones concurrentes;
- reinicios en bucle si una migracion tarda mas que el health check;
- arranques parcialmente exitosos con schemas desalineados.

### 14.5 Criterio de aceptación de fase

- se puede acceder a tenants distintos en local;
- staging reproduce el comportamiento multi-tenant real;
- Render builda el mismo artefacto que fue validado localmente;
- las migraciones se validan como paso separado del web start.

---

## 15. Fase 10 — Cloud readiness: Render, Resend y R2

### 15.1 Objetivo

Cerrar el gap entre "funciona local" y "es operable en produccion".

### 15.2 Render

Validar:

- un solo `render.yaml` productivo;
- Dockerfile con contexto real de Render;
- build frontend incluido en `/app/react_frontend`;
- `start.prod.sh` sin bootstrap local, sin superusuario automatico y sin credenciales en logs;
- `gunicorn` parametrizado por `$PORT`;
- health check `/api/health/` sin tenant ni sesion.

Las migraciones deben validarse como comando separado de release/predeploy o job, no como parte obligatoria del web process.

### 15.3 Resend

Validar:

- backend SMTP real solo en prod;
- variables obligatorias sin defaults silenciosos;
- activacion y password reset entregados en staging;
- links `https` sin `localhost`, sin host interno Render y sin dominio hardcodeado no confirmado.

### 15.4 R2

Validar:

- pgdump por schema sube a R2;
- logo de negocio sube a R2 con path tenant-safe;
- certificados y claves ARCA suben a R2 con path tenant-safe;
- objetos sensibles privados por defecto;
- Tenant B no puede obtener ni solicitar objetos de Tenant A.

### 15.5 URLs publicas

Centralizar la construccion de URLs publicas en helper o servicio.

No alcanza con corregir emails: tambien deben corregirse respuestas API usadas por frontend para redireccionar a tenant, login bridge, reset y activacion.

### 15.6 Criterio de aceptacion de fase

- deploy Render reproducible;
- email real Resend probado;
- R2 real probado por tipo de objeto sensible;
- URLs publicas consistentes y `https`;
- no quedan hardcodes productivos de dominio salvo decision documentada.

---

## 16. Fase 11 — Endurecimiento funcional y revisión integral

### 16.1 Objetivo

Evitar sectores no contemplados.

### 16.2 Regla operativa

Antes de considerar la V1 completa, se debe ejecutar una revisión transversal módulo por módulo.

### 16.3 Checklist transversal obligatorio

#### Autenticación

- login publico con cuenta global;
- token puente de un solo uso;
- login tenant
- logout tenant
...
- backup tenant A
- backup tenant B
- subida de pgdump a R2
- acceso denegado cruzado a pgdump
- nombre correcto del archivo

#### Cloud

- Render build/deploy
- comando de migraciones separado
- health check
- Resend activacion/reset
- R2 logos/certificados/pgdump
- URLs publicas `https`
#### Public SaaS
...
- `backend/acceso_publico/...` para cuenta global transitoria, si aun no existe
...
## 18. Archivos existentes que casi seguro deben cambiar
...
- `backend/acceso_publico/views.py`
- `backend/acceso_publico/services.py`
...
## 19. Riesgos técnicos concretos que el plan contempla

### 19.1 Riesgo: `Ferreteria.objects.first()`
...
### 19.2 Riesgo: archivos compartidos
...
### 19.3 Riesgo: register historico crea usuarios fuera del modelo SaaS

Mitigación:

- reemplazar el register por onboarding de negocio;
- crear cuenta global transitoria;
- vincularla a un unico usuario tenant operativo.

### 19.4 Riesgo: subdominio inseguro o mutable
...
### 19.5 Riesgo: trial sin enforcement
...
### 19.6 Riesgo: staging no reproduce producción

Mitigación:

- wildcard domain desde el inicio.

### 19.7 Riesgo: migraciones dentro del web start

Mitigacion:

- ejecutar `migrate_schemas` como release/predeploy o job controlado;
- dejar el proceso web limitado a servir `gunicorn`;
- validar que un reinicio del web no reintente migraciones.

### 19.8 Riesgo: contexto Docker distinto al de Render

Mitigacion:

- documentar root operativo exacto;
- ejecutar `docker build` con el mismo contexto que Render;
- hacer fallar el build si falta `react_frontend/index.html`.

### 19.9 Riesgo: URLs publicas parciales

Mitigacion:

- centralizar `PUBLIC_BASE_URL`/dominio tenant;
- validar emails y respuestas API;
- prohibir `http://`, `localhost` y hosts internos en prod.

### 19.10 Riesgo: R2 publico para objetos sensibles

Mitigacion:

- objetos sensibles privados por defecto;
- validar URLs firmadas o acceso mediado por API;
- cubrir pgdump, logos y certificados ARCA, no solo un upload generico.

---

## 20. Riesgos que el plan no resuelve por diseño
...
9. Render, Resend y R2 estan validados con evidencia real;
10. los datos generados en beta son preservables.

---

## 22. Qué no se debe hacer durante la implementación
...
- no guardar pgdump productivos solo en filesystem efimero;
- no hacer publicos pgdump, certificados o claves ARCA;
- no correr migraciones como parte permanente del arranque web;
- no validar Docker con un contexto distinto al de Render;
- no emitir URLs productivas con `http://`, `localhost`, hosts internos o dominios hardcodeados;
- no asumir que un módulo “debería funcionar” sin validarlo.

---

## 23. Estrategia de validación obligatoria

### 23.1 Validación automatizada mínima
...
- bloqueo por setup
- backup por schema
- backup por schema subido a R2
- login publico con cuenta global y puente a usuario tenant
- build Docker/Render
- Resend SMTP

### 23.2 Validación manual obligatoria

- dos tenants distintos simultáneos;
- productos y ventas aislados;
- logos aislados;
- certificados ARCA aislados;
- pgdump en R2 aislado;
- trial vencido;
...
## 24. Recomendación operativa para ejecución
...
## 25. Resultado esperado al finalizar este plan
