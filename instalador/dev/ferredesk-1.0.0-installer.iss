...
begin
  ...
  begin
    // Intentar detener contenedores en la carpeta del proyecto
    // La estructura es {app}\ferredesk
    AppDir := ExpandConstant('{app}\ferredesk');
  ...
  begin
    ...
    // Borrar datos globales en ProgramData
    DelTree(ExpandConstant('{commonappdata}\FerreDesk'), True, True, True);
    
    // Asegurar borrado de carpeta de instalación (por si [UninstallDelete] falló)
    InstallDir := ExpandConstant('{localappdata}\Programs\FerreDesk');
