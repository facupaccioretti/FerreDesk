...
<loginTicketRequest version="1.0">
    ...
    def _preparar_datos_venta_arca(self, venta: Venta) -> Dict[str, Any]:
        """
        Prepara los datos de una venta para envío a ARCA.
        
        Args:
            venta: Instancia del modelo Venta
            
        Returns:
            Dict con datos formateados para ARCA
        """
        try:
            # Obtener datos del comprobante
            comprobante = venta.comprobante
            if not comprobante:
                raise FerreDeskARCAError("Venta sin comprobante asociado")
            
            # Verificar que sea un comprobante fiscal
            if not debe_emitir_arca(comprobante.tipo):
                raise FerreDeskARCAError(f"Comprobante {comprobante.tipo} no requiere emisión ARCA")
            
            # Obtener código ARCA
            codigo_afip = obtener_codigo_arca(comprobante.tipo, comprobante.letra)
            if not codigo_afip:
                raise FerreDeskARCAError(f"No se pudo determinar código ARCA para {comprobante.tipo} {comprobante.letra}")
            
            # Obtener cliente y sus datos
            cliente = venta.ven_idcli
            if not cliente:
                raise FerreDeskARCAError("Venta sin cliente asociado")
            
            # Obtener datos completos desde las vistas
            from ferreapps.ventas.models import VentaCalculada, VentaDetalleItemCalculado, VentaIVAAlicuota
            venta_calculada = VentaCalculada.objects.get(ven_id=venta.ven_id)
            items_calculados = VentaDetalleItemCalculado.objects.filter(vdi_idve=venta.ven_id)
            if not items_calculados.exists():
                raise FerreDeskARCAError("Venta sin items calculados")
            
            # Obtener alícuotas de IVA de la venta
            alicuotas_venta = VentaIVAAlicuota.objects.filter(vdi_idve=venta.ven_id)
            if not alicuotas_venta.exists():
                raise FerreDeskARCAError("Venta sin alícuotas de IVA calculadas")
            
            # Determinar tipo de documento y número según el cliente
            cuit_cliente = cliente.cuit or venta.ven_cuit or ""
            if cuit_cliente and len(cuit_cliente) == 11:
                doc_tipo = 80  # CUIT
                doc_numero = int(cuit_cliente)
            else:
            ...
            condicion_normalizada = normalizar_situacion_iva(condicion_iva_cliente)
            condicion_iva_id = mapear_condicion_iva_afip(condicion_normalizada)
            
            logger.info(f"Cliente: {cliente.razon}, CUIT: {cuit_cliente}, Condición IVA: {condicion_iva_cliente} -> {condicion_normalizada} -> ID: {condicion_iva_id}")
            
            # Preparar datos del comprobante según estructura AFIP
            datos_comprobante = {
                ...
                'ImpTotConc': 0.0,  # 0 para Factura B
                'ImpOpEx': 0.0,
                'ImpTrib': 0.0,
                'FchServDesde': None,
                'FchServHasta': None,
            ...
            # Lógica específica según tipo de comprobante
            if codigo_afip == 11:  # Factura C - Sin IVA
                datos_comprobante.update({
                    'ImpNeto': float(venta_calculada.ven_total),  # Total sin IVA
                    'ImpIVA': 0.0,  # Sin IVA para Factura C
                    'ImpTotal': float(venta_calculada.ven_total),  # Total = Neto
                    'Iva': None  # No informar IVA para Factura C
                })
            else:  # Factura A/B - Con IVA
                # Mapear porcentajes de alícuota a IDs de AFIP
                def obtener_id_afip_por_porcentaje(porcentaje):
                    """Mapea el porcentaje de alícuota al ID correspondiente de AFIP"""
                    mapeo_alicuotas = {
                        0: 1,      # 0% = NO GRAVADO
                        10.5: 4,   # 10.5%
                        21: 5,     # 21%
                ...
                # Construir array de alícuotas dinámicamente
                alicuotas_afip = []
                for alicuota in alicuotas_venta:
                    ...
                    logger.info(f"Alícuota: {alicuota.ali_porce}% -> ID AFIP: {id_afip}, BaseImp: ${alicuota.neto_gravado}, Importe: ${alicuota.iva_total}")
                
                logger.info(f"Total alícuotas a enviar: {len(alicuotas_afip)}")
                ...
                datos_comprobante.update({
                    ...
                    'Iva': {
                        'AlicIva': alicuotas_afip
                    }
                })
            
            # Obtener el último número autorizado de AFIP para este tipo de comprobante
            numero_afip = self._obtener_ultimo_numero_autorizado(codigo_afip)
            
            # Actualizar el número de la venta con el número correcto de AFIP
            venta.ven_numero = numero_afip
            venta.save()
            
            logger.info(f"Número actualizado para venta {venta.ven_id}: {numero_afip}")
            
            # Actualizar los números en los datos del comprobante
            datos_comprobante['CbteDesde'] = numero_afip
            datos_comprobante['CbteHasta'] = numero_afip
            
            return {
                'comprobante': datos_comprobante,
                'tipo_cbte': codigo_afip
            }
            
        except Exception as e:
            raise FerreDeskARCAError(f"Error preparando datos para ARCA: {e}")
    ...
    def emitir_automatico(self, venta: Venta) -> Dict[str, Any]:
        ...
        try:
            ...
            datos_arca = self._preparar_datos_venta_arca(venta)
            
            # Emitir comprobante
            resultado_arca = self._emitir_comprobante_wsfev1(datos_arca)
            ...
            # Actualizar venta con datos ARCA
