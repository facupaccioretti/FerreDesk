...
const PresupuestosManager = () => {
  ...
  // Mostrar errores como mensajes del navegador si no son de ARCA (que ya tiene su propio modal)
  useEffect(() => {
    if (ventasError) {
      const esErrorARCA = ["arca", "afip", "cae", "padrón", "padron", "fiscal"].some(keyword =>
        ventasError.toLowerCase().includes(keyword)
      );
