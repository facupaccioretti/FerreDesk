...
const opcionesMetrica = [
  ...
  { value: "cantidad", label: "Productos" },
  { value: "frecuencia", label: "Compras" },
]

const truncar = (texto = "", maximo = 34) =>
  texto.length > maximo ? `${texto.slice(0, maximo - 1)}…` : texto

const ClientesMasVentasSimple = ({
  tipoMetrica = "total",
  onMetricaChange,
  data = [],
  loading = false,
  ...
  const formatValue = (value) => {
    if (tipoMetrica === "total") return `$${Number(value).toLocaleString("es-AR")}`
    if (tipoMetrica === "cantidad") return `${Number(value).toLocaleString("es-AR")} productos`
    return `${Number(value).toLocaleString("es-AR")} compras`
  }
    ...
    return (
      <div className="flex h-[190px] items-center justify-center">
        <div className="text-center">
          <div className="mx-auto mb-3 h-8 w-8 animate-spin rounded-full border-b-2 border-blue-500" />
  ...
  if (error) {
    return (
      <div className="flex h-[190px] items-center justify-center">
        <div className="text-center">
          <p className="text-sm text-slate-300">No se pudieron cargar los clientes.</p>
          <button
            onClick={onRetry}
    ...
    <div className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">Top clientes</p>
          <p className="truncate text-lg font-bold leading-tight text-slate-100">{truncar(topLabel, 42)}</p>
          <p className="mt-0.5 text-xs text-slate-400">{formatValue(topValue)}</p>
        </div>
        <select
          value={tipoMetrica}
      ...
      <div className="space-y-2">
        {data.slice(0, 5).map((item, index) => {
          ...
          return (
            <div key={`${item.name}-${index}`} className="grid grid-cols-[minmax(0,1fr)_96px] items-center gap-3">
              <div className="min-w-0">
...
export default ClientesMasVentasSimple
