...
const FacturaForm = ({
  ...
  const [form, setForm] = useState(() => {
    const savedForm = localStorage.getItem('facturaFormDraft');
  ...
  useEffect(() => {
    if (!initialData) {
      localStorage.setItem('facturaFormDraft', JSON.stringify(form));
    }
  }, [form, initialData]);
  ...
  return (
    <form className="max-w-4xl w-full mx-auto py-8 px-8 bg-white rounded-xl shadow relative" onSubmit={handleSubmit}>
      <h3 className="text-xl font-semibold text-gray-800 mb-6">{initialData ? (isReadOnly ? 'Ver Factura' : 'Editar Factura') : 'Nueva Factura'}</h3>
      {isReadOnly && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          <label className="block text-sm font-medium text-gray-500 mb-1">N° Factura</label>
      ...
      <div className="mb-8">
        <h4 className="text-lg font-medium text-gray-800 mb-4">Ítems de la Factura</h4>
      ...
      <div className="mt-8 flex justify-end space-x-3">
        ...
        {!isReadOnly && (
          <button type="submit" className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors">{initialData ? 'Guardar Cambios' : 'Crear Factura'}</button>
        )}
      </div>
    </form>
  );
