...
export default function DetalleComprobanteProveedorModal({ open, onClose, itemBase }) {
    ...
    const { getDetalleComprobanteProveedor, eliminarImputacion } = useCuentaCorrienteProveedorAPI()
    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            ...
            setError(null)
            try {
                const detalle = await getDetalleComprobanteProveedor(comprobanteId, itemBase?.comprobante_tipo)
        ...
        cargar()
        return () => { activo = false }
    }, [open, comprobanteId, itemBase?.comprobante_tipo, getDetalleComprobanteProveedor])
                                                ...
                                                <table className="min-w-full text-sm">
                                                    <thead>
                                                        <tr className={`text-left border-b border-slate-200 bg-slate-50`}>
                                                            ...
                                                            <th className="px-4 py-2 text-center text-[10px] uppercase tracking-wide text-slate-500">Acciones</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody className="divide-y divide-slate-100">
                                                        {asociados.length === 0 ? (
                                                            <tr><td className="py-4 text-slate-400 text-center italic" colSpan={4}>No hay imputaciones asociadas</td></tr>
                                                        ) : asociados.map((a, idx) => (
