...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('ventas', '0040_fix_view_calculation_formula'),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- Eliminar las vistas en orden inverso de dependencias
            DROP VIEW IF EXISTS "VENTA_CALCULADO";
            DROP VIEW IF EXISTS "VENTAIVA_ALICUOTA";
            DROP VIEW IF EXISTS "VENTADETALLEITEM_CALCULADO";

            -- Recrear vista de detalle con nuevos campos y redondeo
            ...
            WITH PreciosBase AS (
                SELECT
                    ...
                    vdi."VDI_DETALLE2",
                    vdi."VDI_IDALIIVA",
                    vdi."VDI_PRECIO_UNITARIO_FINAL", -- precio con IVA guardado
                    v."VEN_DESCU1",
                    v."VEN_DESCU2",
                    v."VEN_DESCU3",
                    ali."ALI_PORCE",
                    ...
                    -- Precio sin IVA con 4 decimales para evitar redondeo prematuro
            ...
            SELECT
                ...
                -- Precio unitario bonif + desc con IVA final (2 dec)
                ROUND(
                    ROUND((pb.precio_unitario_sin_iva - ROUND(pb.precio_unitario_sin_iva * pb."VDI_BONIFICA" / 100.0, 4)) 
                        * (1 - COALESCE(pb."VEN_DESCU1",0)/100.0)
                        * (1 - COALESCE(pb."VEN_DESCU2",0)/100.0)
                ...
                -- Precio unitario bonif+desc sin IVA a 2 dec (para totales netos)
                ROUND(
                ...
                -- Subtotales y totales con 2 decimales (ya con descuentos)
                ROUND(
                    ((pb.precio_unitario_sin_iva - ROUND(pb.precio_unitario_sin_iva * pb."VDI_BONIFICA" / 100.0, 4))
                        * (1 - COALESCE(pb."VEN_DESCU1",0)/100.0)
                        * (1 - COALESCE(pb."VEN_DESCU2",0)/100.0)
                ...
                ROUND(
                    ROUND(pb.precio_unitario_sin_iva * pb."ALI_PORCE" / 100.0, 4)
                    * (1 - COALESCE(pb."VEN_DESCU1",0)/100.0)
                    * (1 - COALESCE(pb."VEN_DESCU2",0)/100.0)
                    * (1 - COALESCE(pb."VEN_DESCU3",0)/100.0)
                    * pb."VDI_CANTIDAD"
                ...
                ROUND(
                    ...
                    * pb."VDI_CANTIDAD"
                , 2) AS total_item,
                -- Márgenes con 3 decimales (uso interno)
