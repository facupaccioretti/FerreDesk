...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('ventas', '0019_quitar_jsonb_object_agg_y_iva_desglose'),
    ]
    ...
    operations = [
        migrations.RunSQL(
            ...
            SELECT
                ...
                ROUND(SUM(d.vdi_importe_total), 2) AS subtotal_bruto,
                ROUND(SUM(d.vdi_importe_total) * (1 - v."VEN_DESCU1" / 100.0) * (1 - v."VEN_DESCU2" / 100.0), 2) AS ven_impneto,
                ROUND(COALESCE(SUM(i.iva_total), 0), 2) AS iva_global,
                ROUND((SUM(d.vdi_importe_total) * (1 - v."VEN_DESCU1" / 100.0) * (1 - v."VEN_DESCU2" / 100.0)) + COALESCE(SUM(i.iva_total), 0), 2) AS ven_total
            FROM
                "VENTA" v
